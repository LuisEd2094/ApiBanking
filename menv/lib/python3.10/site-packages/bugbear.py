    ...
    def run(self):
        ...
        if self.options and hasattr(self.options, "extend_immutable_calls"):
            b008_b039_extend_immutable_calls = set(
                self.options.extend_immutable_calls)
        else:
            b008_b039_extend_immutable_calls = set()
        ...
        if self.options and hasattr(self.options, "classmethod_decorators"):
            b902_classmethod_decorators = set(
                self.options.classmethod_decorators)
    ...
    def gen_line_based_checks(self):
        ...
        for lineno, line in enumerate(self.lines, start=1):
            ...
            if no_comment_line != line:
                no_comment_line = noqa_type_ignore_regex.sub(
                    "", no_comment_line)
            ...
            if length > 1.1 * self.max_line_length and no_comment_line.strip():
                ...
                is_line_comment_url_path = len(
                    chunks) == 2 and chunks[0] == "#"

                just_long_url_path = len(chunks) == 1

                num_leading_whitespaces = len(
                    no_comment_line) - len(chunks[-1])
...
class BugBearVisitor(ast.NodeVisitor):
    ...
    def check_for_b005(self, node) -> None:
        if isinstance(node, ast.Import):
            ...
            for name in node.names:
                self._b005_imports.add(
                    f"{node.module}.{name.name or name.asname}")
    ...
    def check_for_b013_b029_b030(self, node: ast.ExceptHandler) -> list[str]:
        ...
        ignored_handlers: list[ast.Name |
                               ast.Attribute | ast.Call | ast.Starred] = []
        ...
        for node in ast.walk(loop_node):
            ...
            if isinstance(node, FUNCTION_NODES) and node not in safe_functions:
                ...
                if isinstance(node, ast.Lambda):
                    ...
                    body_nodes = itertools.chain.from_iterable(
                        map(ast.walk, node.body))
                errors = []
    ...
    def check_for_b024_and_b027(self, node: ast.ClassDef) -> None:  # noqa: C901
        ...
        if has_method and not has_abstract_method:
            self.errors.append(
                B024(node.lineno, node.col_offset, vars=(node.name,)))
            ...
            if (isinstance(node.func, ast.Name) and node.func.id in ("groupby",)) or (
                ...
                for node in walk_list(loop_node.body):
                    ...
                    if isinstance(node, ast.Name) and node.id == group_name:
                        ...
                        if num_usages > 1:
                            self.errors.append(
                                B031(node.lineno, node.col_offset,
                                     vars=(node.id,))
                            )
    ...
    def check_for_b902(  # noqa: C901 (too complex)
        ...
        if actual_first_arg not in expected_first_args:
            ...
            self.errors.append(
                B902(lineno, col, vars=(actual_first_arg,
                     kind, expected_first_args[0]))
            )
    ...
    def check_for_b025(self, node) -> None:
        ...
        for duplicate in duplicates:
            self.errors.append(
                B025(node.lineno, node.col_offset, vars=(duplicate,)))
    ...
    def check_for_b034(self, node: ast.Call) -> None:
        ...
        def check(num_args: int, param_name: str) -> None:
            if len(node.args) > num_args:
                ...
                self.errors.append(
                    B034(arg.lineno, arg.col_offset,
                         vars=(func.attr, param_name))
                )
...
class FunctionDefDefaultsVisitor(ast.NodeVisitor):
    ...
    def __init__(
        ...
        for node in B006_MUTABLE_LITERALS + B006_MUTABLE_COMPREHENSIONS:
            setattr(self, f"visit_{node}",
                    self.visit_mutable_literal_or_comprehension)
    ...
    def visit_mutable_literal_or_comprehension(self, node) -> None:
        ...
        if self.arg_depth == 1:
            self.errors.append(self.error_code_calls(
    ...
    def visit_Call(self, node) -> None:
        ...
        if call_path == "float" and len(node.args) == 1:
            ...
            self.errors.append(self.error_code_literals(
                node.lineno, node.col_offset))
...
B023 = Error(
    message="B023 Function definition does not bind loop variable {!r}.")
B024 = Error(
    message=(
...
B902_IMPLICIT_CLASSMETHODS = {"__new__",
