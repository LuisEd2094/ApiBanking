...
class Connection:
    ...
    def _send_autocommit_mode(self):
        ...
        self._execute_command(
            COMMAND.COM_QUERY, "SET AUTOCOMMIT = %s" % self.escape(
                self.autocommit_mode)
        )
    ...
    def _request_authentication(self):
        ...
        if self.ssl and self.server_capabilities & CLIENT.SSL:
            ...
            self._sock = self.ctx.wrap_socket(
        ...
        if auth_packet.is_auth_switch_request():
            ...
            if self._auth_plugin_name == "caching_sha2_password":
                auth_packet = _auth.caching_sha2_password_auth(
                    self, auth_packet)
            elif self._auth_plugin_name == "sha256_password":
    ...
    def _process_auth(self, plugin_name, auth_packet):
        ...
        if plugin_name == b"caching_sha2_password":
            ...
            data = _auth.scramble_native_password(
                self.password, auth_packet.read_all())
        elif plugin_name == b"client_ed25519":
            data = _auth.ed25519_password(
                self.password, auth_packet.read_all())
    ...
    def _get_auth_plugin_handler(self, plugin_name):
        ...
        if not plugin_class and isinstance(plugin_name, bytes):
            plugin_class = self._auth_plugin_map.get(
    ...
    def _get_server_information(self):
        ...
        if len(data) >= i + 6:
            lang, stat, cap_h, salt_len = struct.unpack(
                "<BHHB", data[i: i + 6])
            i += 6
